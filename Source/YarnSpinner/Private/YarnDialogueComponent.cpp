...
// Called every frame
void UYarnDialogueComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UYarnDialogueComponent::OnDialogueStarted_Implementation()
{
    OnDialogueEndedDelegate.Broadcast();
}


void UYarnDialogueComponent::OnDialogueEnded_Implementation()
{
    OnDialogueEndedDelegate.Broadcast();
}


void UYarnDialogueComponent::OnRunLine_Implementation(ULine* Line, const TArray<TSoftObjectPtr<UObject>>& LineAssets)
{
    // default = log and immediately continue
	if (!OnRunLineDelegate.IsBound())
	...
	OnRunLineDelegate.Broadcast(Line, LineAssets);
}


void UYarnDialogueComponent::OnRunOptions_Implementation(const TArray<class UOption*>& Options)
{
    // default = log and choose the first option
	if (!OnRunOptionsDelegate.IsBound())
	{
	...
	OnRunOptionsDelegate.Broadcast(Options);
}


void UYarnDialogueComponent::OnRunCommand_Implementation(const FString& Command, const TArray<FString>& Parameters)
{
    // default = no-op
	if (!OnRunCommandDelegate.IsBound())
	{
	...
	{
		//Use delegate return value to decide whether to continue
		if (!OnRunCommandDelegate.Execute(Command, Parameters))
		{
			ContinueDialogue();
		}
	}
...
void UYarnDialogueComponent::StartDialogue(FName NodeName)
{
...
void UYarnDialogueComponent::ContinueDialogue()
{
    YS_LOG_FUNCSIG
...
void UYarnDialogueComponent::SelectOption(UOption* Option)
{
...
void UYarnDialogueComponent::Log(std::string Message, Type Severity)
{
...
void UYarnDialogueComponent::SetValue(std::string Name, bool bValue)
{
...
void UYarnDialogueComponent::SetValue(std::string Name, float Value)
{
...
void UYarnDialogueComponent::SetValue(std::string Name, std::string Value)
{
...
bool UYarnDialogueComponent::HasValue(std::string Name)
{
    return YarnSubsystem()->HasValue(Name);
}


Yarn::Value UYarnDialogueComponent::GetValue(std::string Name)
{
...
void UYarnDialogueComponent::ClearValue(std::string Name)
{
...
UYarnSubsystem* UYarnDialogueComponent::YarnSubsystem() const
{
    ...
    return GetOwner()->GetGameInstance()->GetSubsystem<UYarnSubsystem>();
}


void UYarnDialogueComponent::GetDisplayTextForLine(ULine* Line, const Yarn::Line& YarnLine)
{
...
void UYarnDialogueComponent::InitializeComponent()
{
    ...
    // configuring it to use our library, plus use this UYarnDialogueComponent as the
    ...
    {
        UE_LOG(LogYarnSpinner, Log, TEXT("Received %llu options"), OptionSet.Options.size());
